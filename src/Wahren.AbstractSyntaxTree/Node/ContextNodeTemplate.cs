#nullable enable
// <auto-generated>
// THIS (.cs) FILE IS GENERATED BY T4. DO NOT CHANGE IT. CHANGE THE .tt FILE INSTEAD.
// </auto-generated>
namespace Wahren.AbstractSyntaxTree.Node;

public sealed class ContextNode : INode
{
    public uint Kind { get; set; }
    public uint BracketLeft { get; set; }
    public uint BracketRight { get; set; }

    public Pair_NullableString_NullableInt_ArrayElement? _war;
    public Pair_NullableString_NullableInt_ArrayElement? camp;
    public Pair_NullableString_NullableInt_ArrayElement? _help;
    public Pair_NullableString_NullableInt_ArrayElement? _load;
    public Pair_NullableString_NullableInt_ArrayElement? _menu;
    public Pair_NullableString_NullableInt_ArrayElement? _name;
    public Pair_NullableString_NullableInt_ArrayElement? _save;
    public Pair_NullableString_NullableInt_ArrayElement? _spot;
    public Pair_NullableString_NullableInt_ArrayElement? _tool;
    public Pair_NullableString_NullableInt_ArrayElement? _war2;
    public Pair_NullableString_NullableInt_ArrayElement? bloom;
    public Pair_NullableString_NullableInt_ArrayElement? _corps;
    public Pair_NullableString_NullableInt_ArrayElement? _diplo;
    public Pair_NullableString_NullableInt_ArrayElement? _help2;
    public Pair_NullableString_NullableInt_ArrayElement? _load2;
    public Pair_NullableString_NullableInt_ArrayElement? _menu2;
    public Pair_NullableString_NullableInt_ArrayElement? _merce;
    public Pair_NullableString_NullableInt_ArrayElement? _name2;
    public Pair_NullableString_NullableInt_ArrayElement? _save2;
    public Pair_NullableString_NullableInt_ArrayElement? _spot2;
    public Pair_NullableString_NullableInt_ArrayElement? _tool2;
    public Pair_NullableString_NullableInt_ArrayElement? discus;
    public Pair_NullableString_NullableInt_ArrayElement? font10;
    public Pair_NullableString_NullableInt_ArrayElement? font12;
    public Pair_NullableString_NullableInt_ArrayElement? font14;
    public Pair_NullableString_NullableInt_ArrayElement? font16;
    public Pair_NullableString_NullableInt_ArrayElement? font20;
    public Pair_NullableString_NullableInt_ArrayElement? member;
    public Pair_NullableString_NullableInt_ArrayElement? _battle;
    public Pair_NullableString_NullableInt_ArrayElement? _corps2;
    public Pair_NullableString_NullableInt_ArrayElement? _detail;
    public Pair_NullableString_NullableInt_ArrayElement? _dialog;
    public Pair_NullableString_NullableInt_ArrayElement? _diplo2;
    public Pair_NullableString_NullableInt_ArrayElement? _merce2;
    public Pair_NullableString_NullableInt_ArrayElement? _spot22;
    public Pair_NullableString_NullableInt_ArrayElement? _status;
    public Pair_NullableString_NullableInt_ArrayElement? _battle2;
    public Pair_NullableString_NullableInt_ArrayElement? _bstatus;
    public Pair_NullableString_NullableInt_ArrayElement? _commenu;
    public Pair_NullableString_NullableInt_ArrayElement? _detail2;
    public Pair_NullableString_NullableInt_ArrayElement? _dialog2;
    public Pair_NullableString_NullableInt_ArrayElement? _message;
    public Pair_NullableString_NullableInt_ArrayElement? _status2;
    public Pair_NullableString_NullableInt_ArrayElement? btl_auto;
    public Pair_NullableString_NullableInt_ArrayElement? gain_per;
    public Pair_NullableString_NullableInt_ArrayElement? loyal_up;
    public Pair_NullableString_NullableInt_ArrayElement? max_unit;
    public Pair_NullableString_NullableInt_ArrayElement? my_range;
    public Pair_NullableString_NullableInt_ArrayElement? npm_play;
    public Pair_NullableString_NullableInt_ArrayElement? politics;
    public Pair_NullableString_NullableInt_ArrayElement? raid_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? raid_coe;
    public Pair_NullableString_NullableInt_ArrayElement? raid_max;
    public Pair_NullableString_NullableInt_ArrayElement? raid_min;
    public Pair_NullableString_NullableInt_ArrayElement? ring_red;
    public Pair_NullableString_NullableInt_ArrayElement? trust_up;
    public Pair_NullableString_NullableInt_ArrayElement? win_gain;
    public Pair_NullableString_NullableInt_ArrayElement? _bstatus2;
    public Pair_NullableString_NullableInt_ArrayElement? _commenu2;
    public Pair_NullableString_NullableInt_ArrayElement? _heromenu;
    public Pair_NullableString_NullableInt_ArrayElement? _message2;
    public Pair_NullableString_NullableInt_ArrayElement? _scenario;
    public Pair_NullableString_NullableInt_ArrayElement? actor_per;
    public Pair_NullableString_NullableInt_ArrayElement? btl_limit;
    public Pair_NullableString_NullableInt_ArrayElement? color_red;
    public Pair_NullableString_NullableInt_ArrayElement? font_face;
    public Pair_NullableString_NullableInt_ArrayElement? font_file;
    public Pair_NullableString_NullableInt_ArrayElement? font_text;
    public Pair_NullableString_NullableInt_ArrayElement? fv_hp_per;
    public Pair_NullableString_NullableInt_ArrayElement? fv_mp_per;
    public Pair_NullableString_NullableInt_ArrayElement? mode_easy;
    public Pair_NullableString_NullableInt_ArrayElement? mode_hard;
    public Pair_NullableString_NullableInt_ArrayElement? mode_luna;
    public Pair_NullableString_NullableInt_ArrayElement? narabijun;
    public Pair_NullableString_NullableInt_ArrayElement? ring_blue;
    public Pair_NullableString_NullableInt_ArrayElement? ring_cyan;
    public Pair_NullableString_NullableInt_ArrayElement? title_bgm;
    public Pair_NullableString_NullableInt_ArrayElement? wnd_alpha;
    public Pair_NullableString_NullableInt_ArrayElement? world_bgm;
    public Pair_NullableString_NullableInt_ArrayElement? _heromenu2;
    public Pair_NullableString_NullableInt_ArrayElement? _powerinfo;
    public Pair_NullableString_NullableInt_ArrayElement? _scenario2;
    public Pair_NullableString_NullableInt_ArrayElement? battle_bgm;
    public Pair_NullableString_NullableInt_ArrayElement? bgm_volume;
    public Pair_NullableString_NullableInt_ArrayElement? btl_healer;
    public Pair_NullableString_NullableInt_ArrayElement? cavalry_ai;
    public Pair_NullableString_NullableInt_ArrayElement? color_blue;
    public Pair_NullableString_NullableInt_ArrayElement? color_cyan;
    public Pair_NullableString_NullableInt_ArrayElement? color_name;
    public Pair_NullableString_NullableInt_ArrayElement? color_text;
    public Pair_NullableString_NullableInt_ArrayElement? damage_max;
    public Pair_NullableString_NullableInt_ArrayElement? damage_min;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_down;
    public Pair_NullableString_NullableInt_ArrayElement? font_title;
    public Pair_NullableString_NullableInt_ArrayElement? force_fire;
    public Pair_NullableString_NullableInt_ArrayElement? free_raise;
    public Pair_NullableString_NullableInt_ArrayElement? loyal_down;
    public Pair_NullableString_NullableInt_ArrayElement? quota_view;
    public Pair_NullableString_NullableInt_ArrayElement? race_label;
    public Pair_NullableString_NullableInt_ArrayElement? ring_alpha;
    public Pair_NullableString_NullableInt_ArrayElement? ring_green;
    public Pair_NullableString_NullableInt_ArrayElement? roam_avoid;
    public Pair_NullableString_NullableInt_ArrayElement? senior_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? spot_fire1;
    public Pair_NullableString_NullableInt_ArrayElement? spot_fire2;
    public Pair_NullableString_NullableInt_ArrayElement? spot_route;
    public Pair_NullableString_NullableInt_ArrayElement? title_name;
    public Pair_NullableString_NullableInt_ArrayElement? wnd_button;
    public Pair_NullableString_NullableInt_ArrayElement? wskin_cell;
    public Pair_NullableString_NullableInt_ArrayElement? zone_alpha;
    public Pair_NullableString_NullableInt_ArrayElement? _personinfo;
    public Pair_NullableString_NullableInt_ArrayElement? _powerinfo2;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_gain;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_turn;
    public Pair_NullableString_NullableInt_ArrayElement? battle_fast;
    public Pair_NullableString_NullableInt_ArrayElement? btl_counter;
    public Pair_NullableString_NullableInt_ArrayElement? btl_limit_c;
    public Pair_NullableString_NullableInt_ArrayElement? btl_minimap;
    public Pair_NullableString_NullableInt_ArrayElement? btl_msgtime;
    public Pair_NullableString_NullableInt_ArrayElement? btl_slowper;
    public Pair_NullableString_NullableInt_ArrayElement? btl_standby;
    public Pair_NullableString_NullableInt_ArrayElement? btl_unitmax;
    public Pair_NullableString_NullableInt_ArrayElement? btl_wingmax;
    public Pair_NullableString_NullableInt_ArrayElement? color_green;
    public Pair_NullableString_NullableInt_ArrayElement? color_white;
    public Pair_NullableString_NullableInt_ArrayElement? compati_bad;
    public Pair_NullableString_NullableInt_ArrayElement? damage_max2;
    public Pair_NullableString_NullableInt_ArrayElement? damage_min2;
    public Pair_NullableString_NullableInt_ArrayElement? font_detail;
    public Pair_NullableString_NullableInt_ArrayElement? fv_move_per;
    public Pair_NullableString_NullableInt_ArrayElement? item_window;
    public Pair_NullableString_NullableInt_ArrayElement? mode_normal;
    public Pair_NullableString_NullableInt_ArrayElement? neutral_max;
    public Pair_NullableString_NullableInt_ArrayElement? neutral_min;
    public Pair_NullableString_NullableInt_ArrayElement? power_order;
    public Pair_NullableString_NullableInt_ArrayElement? race_suffix;
    public Pair_NullableString_NullableInt_ArrayElement? ring_orange;
    public Pair_NullableString_NullableInt_ArrayElement? ring_purple;
    public Pair_NullableString_NullableInt_ArrayElement? ring_yellow;
    public Pair_NullableString_NullableInt_ArrayElement? senior_cost;
    public Pair_NullableString_NullableInt_ArrayElement? sound_count;
    public Pair_NullableString_NullableInt_ArrayElement? talent_mode;
    public Pair_NullableString_NullableInt_ArrayElement? title_bgm_s;
    public Pair_NullableString_NullableInt_ArrayElement? unit_vanish;
    public Pair_NullableString_NullableInt_ArrayElement? zone_radius;
    public Pair_NullableString_NullableInt_ArrayElement? _personinfo2;
    public Pair_NullableString_NullableInt_ArrayElement? _powerselect;
    public Pair_NullableString_NullableInt_ArrayElement? btl_mode_coe;
    public Pair_NullableString_NullableInt_ArrayElement? btl_raidback;
    public Pair_NullableString_NullableInt_ArrayElement? btl_semiauto;
    public Pair_NullableString_NullableInt_ArrayElement? color_orange;
    public Pair_NullableString_NullableInt_ArrayElement? color_yellow;
    public Pair_NullableString_NullableInt_ArrayElement? dead_penalty;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_frd_up;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_sup_up;
    public Pair_NullableString_NullableInt_ArrayElement? employ_range;
    public Pair_NullableString_NullableInt_ArrayElement? font_balloon;
    public Pair_NullableString_NullableInt_ArrayElement? foreign_test;
    public Pair_NullableString_NullableInt_ArrayElement? fv_hprec_per;
    public Pair_NullableString_NullableInt_ArrayElement? fv_level_per;
    public Pair_NullableString_NullableInt_ArrayElement? fv_speed_per;
    public Pair_NullableString_NullableInt_ArrayElement? leave_period;
    public Pair_NullableString_NullableInt_ArrayElement? loyal_border;
    public Pair_NullableString_NullableInt_ArrayElement? merits_bonus;
    public Pair_NullableString_NullableInt_ArrayElement? movement_num;
    public Pair_NullableString_NullableInt_ArrayElement? myhelp_range;
    public Pair_NullableString_NullableInt_ArrayElement? picture_fade;
    public Pair_NullableString_NullableInt_ArrayElement? quarter_view;
    public Pair_NullableString_NullableInt_ArrayElement? skill_offset;
    public Pair_NullableString_NullableInt_ArrayElement? sound_volume;
    public Pair_NullableString_NullableInt_ArrayElement? support_gain;
    public Pair_NullableString_NullableInt_ArrayElement? title_volume;
    public Pair_NullableString_NullableInt_ArrayElement? trust_morale;
    public Pair_NullableString_NullableInt_ArrayElement? unicode_test;
    public Pair_NullableString_NullableInt_ArrayElement? unit_use_coe;
    public Pair_NullableString_NullableInt_ArrayElement? war_capacity;
    public Pair_NullableString_NullableInt_ArrayElement? world_volume;
    public Pair_NullableString_NullableInt_ArrayElement? _personselect;
    public Pair_NullableString_NullableInt_ArrayElement? _powerselect2;
    public Pair_NullableString_NullableInt_ArrayElement? _scenariotext;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_player;
    public Pair_NullableString_NullableInt_ArrayElement? b_sound_count;
    public Pair_NullableString_NullableInt_ArrayElement? battle_volume;
    public Pair_NullableString_NullableInt_ArrayElement? bold_spotpath;
    public Pair_NullableString_NullableInt_ArrayElement? btl_intercept;
    public Pair_NullableString_NullableInt_ArrayElement? btl_lineshift;
    public Pair_NullableString_NullableInt_ArrayElement? btl_minimap_w;
    public Pair_NullableString_NullableInt_ArrayElement? color_detail1;
    public Pair_NullableString_NullableInt_ArrayElement? color_detail2;
    public Pair_NullableString_NullableInt_ArrayElement? color_detail3;
    public Pair_NullableString_NullableInt_ArrayElement? color_magenta;
    public Pair_NullableString_NullableInt_ArrayElement? damage_consti;
    public Pair_NullableString_NullableInt_ArrayElement? event_bg_size;
    public Pair_NullableString_NullableInt_ArrayElement? executive_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? face_colorkey;
    public Pair_NullableString_NullableInt_ArrayElement? font_prologue;
    public Pair_NullableString_NullableInt_ArrayElement? fv_attack_per;
    public Pair_NullableString_NullableInt_ArrayElement? fv_consti_mul;
    public Pair_NullableString_NullableInt_ArrayElement? fv_summon_mul;
    public Pair_NullableString_NullableInt_ArrayElement? item_sell_per;
    public Pair_NullableString_NullableInt_ArrayElement? newpower_flag;
    public Pair_NullableString_NullableInt_ArrayElement? notalent_down;
    public Pair_NullableString_NullableInt_ArrayElement? picture_floor;
    public Pair_NullableString_NullableInt_ArrayElement? picture_trans;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_add;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_all;
    public Pair_NullableString_NullableInt_ArrayElement? spot_capacity;
    public Pair_NullableString_NullableInt_ArrayElement? support_range;
    public Pair_NullableString_NullableInt_ArrayElement? unit_scream_m;
    public Pair_NullableString_NullableInt_ArrayElement? unit_scream_n;
    public Pair_NullableString_NullableInt_ArrayElement? unit_slow_per;
    public Pair_NullableString_NullableInt_ArrayElement? _personselect2;
    public Pair_NullableString_NullableInt_ArrayElement? _scenariotext2;
    public Pair_NullableString_NullableInt_ArrayElement? b_sound_volume;
    public Pair_NullableString_NullableInt_ArrayElement? bold_spotlabel;
    public Pair_NullableString_NullableInt_ArrayElement? btl_attackmode;
    public Pair_NullableString_NullableInt_ArrayElement? btl_battlemode;
    public Pair_NullableString_NullableInt_ArrayElement? btl_castle_lot;
    public Pair_NullableString_NullableInt_ArrayElement? btl_chargemode;
    public Pair_NullableString_NullableInt_ArrayElement? btl_min_damage;
    public Pair_NullableString_NullableInt_ArrayElement? caution_adjust;
    public Pair_NullableString_NullableInt_ArrayElement? color_namehelp;
    public Pair_NullableString_NullableInt_ArrayElement? color_prologue;
    public Pair_NullableString_NullableInt_ArrayElement? default_ending;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_cost_per;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_frd_down;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_mode_itv;
    public Pair_NullableString_NullableInt_ArrayElement? executive_cost;
    public Pair_NullableString_NullableInt_ArrayElement? font_spotlabel;
    public Pair_NullableString_NullableInt_ArrayElement? font_text_size;
    public Pair_NullableString_NullableInt_ArrayElement? fullbody_speak;
    public Pair_NullableString_NullableInt_ArrayElement? mode_easy_text;
    public Pair_NullableString_NullableInt_ArrayElement? mode_hard_text;
    public Pair_NullableString_NullableInt_ArrayElement? mode_luna_text;
    public Pair_NullableString_NullableInt_ArrayElement? picture_center;
    public Pair_NullableString_NullableInt_ArrayElement? raise_yabo_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_item;
    public Pair_NullableString_NullableInt_ArrayElement? t_sound_volume;
    public Pair_NullableString_NullableInt_ArrayElement? target_way_coe;
    public Pair_NullableString_NullableInt_ArrayElement? title_menu_top;
    public Pair_NullableString_NullableInt_ArrayElement? trust_down_per;
    public Pair_NullableString_NullableInt_ArrayElement? unit_drain_mul;
    public Pair_NullableString_NullableInt_ArrayElement? unit_keep_form;
    public Pair_NullableString_NullableInt_ArrayElement? unit_level_max;
    public Pair_NullableString_NullableInt_ArrayElement? unit_status_hp;
    public Pair_NullableString_NullableInt_ArrayElement? b_sound_itv_max;
    public Pair_NullableString_NullableInt_ArrayElement? b_sound_itv_min;
    public Pair_NullableString_NullableInt_ArrayElement? bold_escapepath;
    public Pair_NullableString_NullableInt_ArrayElement? btl_blind_alpha;
    public Pair_NullableString_NullableInt_ArrayElement? btl_prepare_max;
    public Pair_NullableString_NullableInt_ArrayElement? btl_prepare_min;
    public Pair_NullableString_NullableInt_ArrayElement? btl_retreat_coe;
    public Pair_NullableString_NullableInt_ArrayElement? color_spotlabel;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_frd_peace;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_league_up;
    public Pair_NullableString_NullableInt_ArrayElement? fullbody_detail;
    public Pair_NullableString_NullableInt_ArrayElement? unit_action_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? unit_escape_run;
    public Pair_NullableString_NullableInt_ArrayElement? unit_hand_range;
    public Pair_NullableString_NullableInt_ArrayElement? unit_keep_color;
    public Pair_NullableString_NullableInt_ArrayElement? unit_poison_per;
    public Pair_NullableString_NullableInt_ArrayElement? unit_search_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? unit_status_min;
    public Pair_NullableString_NullableInt_ArrayElement? unit_status_rec;
    public Pair_NullableString_NullableInt_ArrayElement? unit_suck_limit;
    public Pair_NullableString_NullableInt_ArrayElement? unit_summon_min;
    public Pair_NullableString_NullableInt_ArrayElement? unit_view_range;
    public Pair_NullableString_NullableInt_ArrayElement? watching_replay;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_gain_lose;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_price_coe;
    public Pair_NullableString_NullableInt_ArrayElement? btl_breast_width;
    public Pair_NullableString_NullableInt_ArrayElement? btl_gate_through;
    public Pair_NullableString_NullableInt_ArrayElement? btl_nocastle_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? btl_replace_line;
    public Pair_NullableString_NullableInt_ArrayElement? circle_alpha_per;
    public Pair_NullableString_NullableInt_ArrayElement? color_spotlabel2;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_bestfriend;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_budget_per;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_most_power;
    public Pair_NullableString_NullableInt_ArrayElement? employ_price_coe;
    public Pair_NullableString_NullableInt_ArrayElement? loyal_escape_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? mode_normal_text;
    public Pair_NullableString_NullableInt_ArrayElement? raise_most_power;
    public Pair_NullableString_NullableInt_ArrayElement? scenario_select2;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_assist;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_leader;
    public Pair_NullableString_NullableInt_ArrayElement? title_menu_right;
    public Pair_NullableString_NullableInt_ArrayElement? title_menu_space;
    public Pair_NullableString_NullableInt_ArrayElement? training_average;
    public Pair_NullableString_NullableInt_ArrayElement? unit_battle_cram;
    public Pair_NullableString_NullableInt_ArrayElement? unit_castle_cram;
    public Pair_NullableString_NullableInt_ArrayElement? unit_image_right;
    public Pair_NullableString_NullableInt_ArrayElement? unit_knock_speed;
    public Pair_NullableString_NullableInt_ArrayElement? unit_sword_slide;
    public Pair_NullableString_NullableInt_ArrayElement? vassal_price_coe;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_money_mini;
    public Pair_NullableString_NullableInt_ArrayElement? arbeit_vassal_coe;
    public Pair_NullableString_NullableInt_ArrayElement? b_sound_no_repeat;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_success_frd;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_success_leg;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_success_sup;
    public Pair_NullableString_NullableInt_ArrayElement? skillicon_special;
    public Pair_NullableString_NullableInt_ArrayElement? spot_escape_route;
    public Pair_NullableString_NullableInt_ArrayElement? support_gain_lose;
    public Pair_NullableString_NullableInt_ArrayElement? support_trust_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? unit_attack_range;
    public Pair_NullableString_NullableInt_ArrayElement? unit_charge_slide;
    public Pair_NullableString_NullableInt_ArrayElement? unit_element_heal;
    public Pair_NullableString_NullableInt_ArrayElement? unit_escape_range;
    public Pair_NullableString_NullableInt_ArrayElement? unit_status_death;
    public Pair_NullableString_NullableInt_ArrayElement? unit_summon_level;
    public Pair_NullableString_NullableInt_ArrayElement? compati_vassal_bdr;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_frd_turndown;
    public Pair_NullableString_NullableInt_ArrayElement? diplo_sup_turndown;
    public Pair_NullableString_NullableInt_ArrayElement? neutral_member_max;
    public Pair_NullableString_NullableInt_ArrayElement? neutral_member_min;
    public Pair_NullableString_NullableInt_ArrayElement? trust_down_support;
    public Pair_NullableString_NullableInt_ArrayElement? unit_illusion_miss;
    public Pair_NullableString_NullableInt_ArrayElement? unit_missile_slide;
    public Pair_NullableString_NullableInt_ArrayElement? unit_retreat_speed;
    public Pair_NullableString_NullableInt_ArrayElement? unit_confusion_miss;
    public Pair_NullableString_NullableInt_ArrayElement? unit_lifebar_height;
    public Pair_NullableString_NullableInt_ArrayElement? unit_retreat_damage;
    public Pair_NullableString_NullableInt_ArrayElement? unit_specialuse_per;
    public Pair_NullableString_NullableInt_ArrayElement? unit_castle_forcefire;
    public void Dispose()
    {
        _war?.Dispose();
        _war = null;
        camp?.Dispose();
        camp = null;
        _help?.Dispose();
        _help = null;
        _load?.Dispose();
        _load = null;
        _menu?.Dispose();
        _menu = null;
        _name?.Dispose();
        _name = null;
        _save?.Dispose();
        _save = null;
        _spot?.Dispose();
        _spot = null;
        _tool?.Dispose();
        _tool = null;
        _war2?.Dispose();
        _war2 = null;
        bloom?.Dispose();
        bloom = null;
        _corps?.Dispose();
        _corps = null;
        _diplo?.Dispose();
        _diplo = null;
        _help2?.Dispose();
        _help2 = null;
        _load2?.Dispose();
        _load2 = null;
        _menu2?.Dispose();
        _menu2 = null;
        _merce?.Dispose();
        _merce = null;
        _name2?.Dispose();
        _name2 = null;
        _save2?.Dispose();
        _save2 = null;
        _spot2?.Dispose();
        _spot2 = null;
        _tool2?.Dispose();
        _tool2 = null;
        discus?.Dispose();
        discus = null;
        font10?.Dispose();
        font10 = null;
        font12?.Dispose();
        font12 = null;
        font14?.Dispose();
        font14 = null;
        font16?.Dispose();
        font16 = null;
        font20?.Dispose();
        font20 = null;
        member?.Dispose();
        member = null;
        _battle?.Dispose();
        _battle = null;
        _corps2?.Dispose();
        _corps2 = null;
        _detail?.Dispose();
        _detail = null;
        _dialog?.Dispose();
        _dialog = null;
        _diplo2?.Dispose();
        _diplo2 = null;
        _merce2?.Dispose();
        _merce2 = null;
        _spot22?.Dispose();
        _spot22 = null;
        _status?.Dispose();
        _status = null;
        _battle2?.Dispose();
        _battle2 = null;
        _bstatus?.Dispose();
        _bstatus = null;
        _commenu?.Dispose();
        _commenu = null;
        _detail2?.Dispose();
        _detail2 = null;
        _dialog2?.Dispose();
        _dialog2 = null;
        _message?.Dispose();
        _message = null;
        _status2?.Dispose();
        _status2 = null;
        btl_auto?.Dispose();
        btl_auto = null;
        gain_per?.Dispose();
        gain_per = null;
        loyal_up?.Dispose();
        loyal_up = null;
        max_unit?.Dispose();
        max_unit = null;
        my_range?.Dispose();
        my_range = null;
        npm_play?.Dispose();
        npm_play = null;
        politics?.Dispose();
        politics = null;
        raid_bdr?.Dispose();
        raid_bdr = null;
        raid_coe?.Dispose();
        raid_coe = null;
        raid_max?.Dispose();
        raid_max = null;
        raid_min?.Dispose();
        raid_min = null;
        ring_red?.Dispose();
        ring_red = null;
        trust_up?.Dispose();
        trust_up = null;
        win_gain?.Dispose();
        win_gain = null;
        _bstatus2?.Dispose();
        _bstatus2 = null;
        _commenu2?.Dispose();
        _commenu2 = null;
        _heromenu?.Dispose();
        _heromenu = null;
        _message2?.Dispose();
        _message2 = null;
        _scenario?.Dispose();
        _scenario = null;
        actor_per?.Dispose();
        actor_per = null;
        btl_limit?.Dispose();
        btl_limit = null;
        color_red?.Dispose();
        color_red = null;
        font_face?.Dispose();
        font_face = null;
        font_file?.Dispose();
        font_file = null;
        font_text?.Dispose();
        font_text = null;
        fv_hp_per?.Dispose();
        fv_hp_per = null;
        fv_mp_per?.Dispose();
        fv_mp_per = null;
        mode_easy?.Dispose();
        mode_easy = null;
        mode_hard?.Dispose();
        mode_hard = null;
        mode_luna?.Dispose();
        mode_luna = null;
        narabijun?.Dispose();
        narabijun = null;
        ring_blue?.Dispose();
        ring_blue = null;
        ring_cyan?.Dispose();
        ring_cyan = null;
        title_bgm?.Dispose();
        title_bgm = null;
        wnd_alpha?.Dispose();
        wnd_alpha = null;
        world_bgm?.Dispose();
        world_bgm = null;
        _heromenu2?.Dispose();
        _heromenu2 = null;
        _powerinfo?.Dispose();
        _powerinfo = null;
        _scenario2?.Dispose();
        _scenario2 = null;
        battle_bgm?.Dispose();
        battle_bgm = null;
        bgm_volume?.Dispose();
        bgm_volume = null;
        btl_healer?.Dispose();
        btl_healer = null;
        cavalry_ai?.Dispose();
        cavalry_ai = null;
        color_blue?.Dispose();
        color_blue = null;
        color_cyan?.Dispose();
        color_cyan = null;
        color_name?.Dispose();
        color_name = null;
        color_text?.Dispose();
        color_text = null;
        damage_max?.Dispose();
        damage_max = null;
        damage_min?.Dispose();
        damage_min = null;
        diplo_down?.Dispose();
        diplo_down = null;
        font_title?.Dispose();
        font_title = null;
        force_fire?.Dispose();
        force_fire = null;
        free_raise?.Dispose();
        free_raise = null;
        loyal_down?.Dispose();
        loyal_down = null;
        quota_view?.Dispose();
        quota_view = null;
        race_label?.Dispose();
        race_label = null;
        ring_alpha?.Dispose();
        ring_alpha = null;
        ring_green?.Dispose();
        ring_green = null;
        roam_avoid?.Dispose();
        roam_avoid = null;
        senior_bdr?.Dispose();
        senior_bdr = null;
        spot_fire1?.Dispose();
        spot_fire1 = null;
        spot_fire2?.Dispose();
        spot_fire2 = null;
        spot_route?.Dispose();
        spot_route = null;
        title_name?.Dispose();
        title_name = null;
        wnd_button?.Dispose();
        wnd_button = null;
        wskin_cell?.Dispose();
        wskin_cell = null;
        zone_alpha?.Dispose();
        zone_alpha = null;
        _personinfo?.Dispose();
        _personinfo = null;
        _powerinfo2?.Dispose();
        _powerinfo2 = null;
        arbeit_gain?.Dispose();
        arbeit_gain = null;
        arbeit_turn?.Dispose();
        arbeit_turn = null;
        battle_fast?.Dispose();
        battle_fast = null;
        btl_counter?.Dispose();
        btl_counter = null;
        btl_limit_c?.Dispose();
        btl_limit_c = null;
        btl_minimap?.Dispose();
        btl_minimap = null;
        btl_msgtime?.Dispose();
        btl_msgtime = null;
        btl_slowper?.Dispose();
        btl_slowper = null;
        btl_standby?.Dispose();
        btl_standby = null;
        btl_unitmax?.Dispose();
        btl_unitmax = null;
        btl_wingmax?.Dispose();
        btl_wingmax = null;
        color_green?.Dispose();
        color_green = null;
        color_white?.Dispose();
        color_white = null;
        compati_bad?.Dispose();
        compati_bad = null;
        damage_max2?.Dispose();
        damage_max2 = null;
        damage_min2?.Dispose();
        damage_min2 = null;
        font_detail?.Dispose();
        font_detail = null;
        fv_move_per?.Dispose();
        fv_move_per = null;
        item_window?.Dispose();
        item_window = null;
        mode_normal?.Dispose();
        mode_normal = null;
        neutral_max?.Dispose();
        neutral_max = null;
        neutral_min?.Dispose();
        neutral_min = null;
        power_order?.Dispose();
        power_order = null;
        race_suffix?.Dispose();
        race_suffix = null;
        ring_orange?.Dispose();
        ring_orange = null;
        ring_purple?.Dispose();
        ring_purple = null;
        ring_yellow?.Dispose();
        ring_yellow = null;
        senior_cost?.Dispose();
        senior_cost = null;
        sound_count?.Dispose();
        sound_count = null;
        talent_mode?.Dispose();
        talent_mode = null;
        title_bgm_s?.Dispose();
        title_bgm_s = null;
        unit_vanish?.Dispose();
        unit_vanish = null;
        zone_radius?.Dispose();
        zone_radius = null;
        _personinfo2?.Dispose();
        _personinfo2 = null;
        _powerselect?.Dispose();
        _powerselect = null;
        btl_mode_coe?.Dispose();
        btl_mode_coe = null;
        btl_raidback?.Dispose();
        btl_raidback = null;
        btl_semiauto?.Dispose();
        btl_semiauto = null;
        color_orange?.Dispose();
        color_orange = null;
        color_yellow?.Dispose();
        color_yellow = null;
        dead_penalty?.Dispose();
        dead_penalty = null;
        diplo_frd_up?.Dispose();
        diplo_frd_up = null;
        diplo_sup_up?.Dispose();
        diplo_sup_up = null;
        employ_range?.Dispose();
        employ_range = null;
        font_balloon?.Dispose();
        font_balloon = null;
        foreign_test?.Dispose();
        foreign_test = null;
        fv_hprec_per?.Dispose();
        fv_hprec_per = null;
        fv_level_per?.Dispose();
        fv_level_per = null;
        fv_speed_per?.Dispose();
        fv_speed_per = null;
        leave_period?.Dispose();
        leave_period = null;
        loyal_border?.Dispose();
        loyal_border = null;
        merits_bonus?.Dispose();
        merits_bonus = null;
        movement_num?.Dispose();
        movement_num = null;
        myhelp_range?.Dispose();
        myhelp_range = null;
        picture_fade?.Dispose();
        picture_fade = null;
        quarter_view?.Dispose();
        quarter_view = null;
        skill_offset?.Dispose();
        skill_offset = null;
        sound_volume?.Dispose();
        sound_volume = null;
        support_gain?.Dispose();
        support_gain = null;
        title_volume?.Dispose();
        title_volume = null;
        trust_morale?.Dispose();
        trust_morale = null;
        unicode_test?.Dispose();
        unicode_test = null;
        unit_use_coe?.Dispose();
        unit_use_coe = null;
        war_capacity?.Dispose();
        war_capacity = null;
        world_volume?.Dispose();
        world_volume = null;
        _personselect?.Dispose();
        _personselect = null;
        _powerselect2?.Dispose();
        _powerselect2 = null;
        _scenariotext?.Dispose();
        _scenariotext = null;
        arbeit_player?.Dispose();
        arbeit_player = null;
        b_sound_count?.Dispose();
        b_sound_count = null;
        battle_volume?.Dispose();
        battle_volume = null;
        bold_spotpath?.Dispose();
        bold_spotpath = null;
        btl_intercept?.Dispose();
        btl_intercept = null;
        btl_lineshift?.Dispose();
        btl_lineshift = null;
        btl_minimap_w?.Dispose();
        btl_minimap_w = null;
        color_detail1?.Dispose();
        color_detail1 = null;
        color_detail2?.Dispose();
        color_detail2 = null;
        color_detail3?.Dispose();
        color_detail3 = null;
        color_magenta?.Dispose();
        color_magenta = null;
        damage_consti?.Dispose();
        damage_consti = null;
        event_bg_size?.Dispose();
        event_bg_size = null;
        executive_bdr?.Dispose();
        executive_bdr = null;
        face_colorkey?.Dispose();
        face_colorkey = null;
        font_prologue?.Dispose();
        font_prologue = null;
        fv_attack_per?.Dispose();
        fv_attack_per = null;
        fv_consti_mul?.Dispose();
        fv_consti_mul = null;
        fv_summon_mul?.Dispose();
        fv_summon_mul = null;
        item_sell_per?.Dispose();
        item_sell_per = null;
        newpower_flag?.Dispose();
        newpower_flag = null;
        notalent_down?.Dispose();
        notalent_down = null;
        picture_floor?.Dispose();
        picture_floor = null;
        picture_trans?.Dispose();
        picture_trans = null;
        skillicon_add?.Dispose();
        skillicon_add = null;
        skillicon_all?.Dispose();
        skillicon_all = null;
        spot_capacity?.Dispose();
        spot_capacity = null;
        support_range?.Dispose();
        support_range = null;
        unit_scream_m?.Dispose();
        unit_scream_m = null;
        unit_scream_n?.Dispose();
        unit_scream_n = null;
        unit_slow_per?.Dispose();
        unit_slow_per = null;
        _personselect2?.Dispose();
        _personselect2 = null;
        _scenariotext2?.Dispose();
        _scenariotext2 = null;
        b_sound_volume?.Dispose();
        b_sound_volume = null;
        bold_spotlabel?.Dispose();
        bold_spotlabel = null;
        btl_attackmode?.Dispose();
        btl_attackmode = null;
        btl_battlemode?.Dispose();
        btl_battlemode = null;
        btl_castle_lot?.Dispose();
        btl_castle_lot = null;
        btl_chargemode?.Dispose();
        btl_chargemode = null;
        btl_min_damage?.Dispose();
        btl_min_damage = null;
        caution_adjust?.Dispose();
        caution_adjust = null;
        color_namehelp?.Dispose();
        color_namehelp = null;
        color_prologue?.Dispose();
        color_prologue = null;
        default_ending?.Dispose();
        default_ending = null;
        diplo_cost_per?.Dispose();
        diplo_cost_per = null;
        diplo_frd_down?.Dispose();
        diplo_frd_down = null;
        diplo_mode_itv?.Dispose();
        diplo_mode_itv = null;
        executive_cost?.Dispose();
        executive_cost = null;
        font_spotlabel?.Dispose();
        font_spotlabel = null;
        font_text_size?.Dispose();
        font_text_size = null;
        fullbody_speak?.Dispose();
        fullbody_speak = null;
        mode_easy_text?.Dispose();
        mode_easy_text = null;
        mode_hard_text?.Dispose();
        mode_hard_text = null;
        mode_luna_text?.Dispose();
        mode_luna_text = null;
        picture_center?.Dispose();
        picture_center = null;
        raise_yabo_bdr?.Dispose();
        raise_yabo_bdr = null;
        skillicon_item?.Dispose();
        skillicon_item = null;
        t_sound_volume?.Dispose();
        t_sound_volume = null;
        target_way_coe?.Dispose();
        target_way_coe = null;
        title_menu_top?.Dispose();
        title_menu_top = null;
        trust_down_per?.Dispose();
        trust_down_per = null;
        unit_drain_mul?.Dispose();
        unit_drain_mul = null;
        unit_keep_form?.Dispose();
        unit_keep_form = null;
        unit_level_max?.Dispose();
        unit_level_max = null;
        unit_status_hp?.Dispose();
        unit_status_hp = null;
        b_sound_itv_max?.Dispose();
        b_sound_itv_max = null;
        b_sound_itv_min?.Dispose();
        b_sound_itv_min = null;
        bold_escapepath?.Dispose();
        bold_escapepath = null;
        btl_blind_alpha?.Dispose();
        btl_blind_alpha = null;
        btl_prepare_max?.Dispose();
        btl_prepare_max = null;
        btl_prepare_min?.Dispose();
        btl_prepare_min = null;
        btl_retreat_coe?.Dispose();
        btl_retreat_coe = null;
        color_spotlabel?.Dispose();
        color_spotlabel = null;
        diplo_frd_peace?.Dispose();
        diplo_frd_peace = null;
        diplo_league_up?.Dispose();
        diplo_league_up = null;
        fullbody_detail?.Dispose();
        fullbody_detail = null;
        unit_action_bdr?.Dispose();
        unit_action_bdr = null;
        unit_escape_run?.Dispose();
        unit_escape_run = null;
        unit_hand_range?.Dispose();
        unit_hand_range = null;
        unit_keep_color?.Dispose();
        unit_keep_color = null;
        unit_poison_per?.Dispose();
        unit_poison_per = null;
        unit_search_bdr?.Dispose();
        unit_search_bdr = null;
        unit_status_min?.Dispose();
        unit_status_min = null;
        unit_status_rec?.Dispose();
        unit_status_rec = null;
        unit_suck_limit?.Dispose();
        unit_suck_limit = null;
        unit_summon_min?.Dispose();
        unit_summon_min = null;
        unit_view_range?.Dispose();
        unit_view_range = null;
        watching_replay?.Dispose();
        watching_replay = null;
        arbeit_gain_lose?.Dispose();
        arbeit_gain_lose = null;
        arbeit_price_coe?.Dispose();
        arbeit_price_coe = null;
        btl_breast_width?.Dispose();
        btl_breast_width = null;
        btl_gate_through?.Dispose();
        btl_gate_through = null;
        btl_nocastle_bdr?.Dispose();
        btl_nocastle_bdr = null;
        btl_replace_line?.Dispose();
        btl_replace_line = null;
        circle_alpha_per?.Dispose();
        circle_alpha_per = null;
        color_spotlabel2?.Dispose();
        color_spotlabel2 = null;
        diplo_bestfriend?.Dispose();
        diplo_bestfriend = null;
        diplo_budget_per?.Dispose();
        diplo_budget_per = null;
        diplo_most_power?.Dispose();
        diplo_most_power = null;
        employ_price_coe?.Dispose();
        employ_price_coe = null;
        loyal_escape_bdr?.Dispose();
        loyal_escape_bdr = null;
        mode_normal_text?.Dispose();
        mode_normal_text = null;
        raise_most_power?.Dispose();
        raise_most_power = null;
        scenario_select2?.Dispose();
        scenario_select2 = null;
        skillicon_assist?.Dispose();
        skillicon_assist = null;
        skillicon_leader?.Dispose();
        skillicon_leader = null;
        title_menu_right?.Dispose();
        title_menu_right = null;
        title_menu_space?.Dispose();
        title_menu_space = null;
        training_average?.Dispose();
        training_average = null;
        unit_battle_cram?.Dispose();
        unit_battle_cram = null;
        unit_castle_cram?.Dispose();
        unit_castle_cram = null;
        unit_image_right?.Dispose();
        unit_image_right = null;
        unit_knock_speed?.Dispose();
        unit_knock_speed = null;
        unit_sword_slide?.Dispose();
        unit_sword_slide = null;
        vassal_price_coe?.Dispose();
        vassal_price_coe = null;
        arbeit_money_mini?.Dispose();
        arbeit_money_mini = null;
        arbeit_vassal_coe?.Dispose();
        arbeit_vassal_coe = null;
        b_sound_no_repeat?.Dispose();
        b_sound_no_repeat = null;
        diplo_success_frd?.Dispose();
        diplo_success_frd = null;
        diplo_success_leg?.Dispose();
        diplo_success_leg = null;
        diplo_success_sup?.Dispose();
        diplo_success_sup = null;
        skillicon_special?.Dispose();
        skillicon_special = null;
        spot_escape_route?.Dispose();
        spot_escape_route = null;
        support_gain_lose?.Dispose();
        support_gain_lose = null;
        support_trust_bdr?.Dispose();
        support_trust_bdr = null;
        unit_attack_range?.Dispose();
        unit_attack_range = null;
        unit_charge_slide?.Dispose();
        unit_charge_slide = null;
        unit_element_heal?.Dispose();
        unit_element_heal = null;
        unit_escape_range?.Dispose();
        unit_escape_range = null;
        unit_status_death?.Dispose();
        unit_status_death = null;
        unit_summon_level?.Dispose();
        unit_summon_level = null;
        compati_vassal_bdr?.Dispose();
        compati_vassal_bdr = null;
        diplo_frd_turndown?.Dispose();
        diplo_frd_turndown = null;
        diplo_sup_turndown?.Dispose();
        diplo_sup_turndown = null;
        neutral_member_max?.Dispose();
        neutral_member_max = null;
        neutral_member_min?.Dispose();
        neutral_member_min = null;
        trust_down_support?.Dispose();
        trust_down_support = null;
        unit_illusion_miss?.Dispose();
        unit_illusion_miss = null;
        unit_missile_slide?.Dispose();
        unit_missile_slide = null;
        unit_retreat_speed?.Dispose();
        unit_retreat_speed = null;
        unit_confusion_miss?.Dispose();
        unit_confusion_miss = null;
        unit_lifebar_height?.Dispose();
        unit_lifebar_height = null;
        unit_retreat_damage?.Dispose();
        unit_retreat_damage = null;
        unit_specialuse_per?.Dispose();
        unit_specialuse_per = null;
        unit_castle_forcefire?.Dispose();
        unit_castle_forcefire = null;
    }

    public ref Pair_NullableString_NullableInt_ArrayElement? TryGet(ReadOnlySpan<char> key)
    {
        if (key.Length < 4)
        {
            goto FALSE;
        }

        var keyULong = StringHashUtility.Calc(key);
        switch (key.Length)
        {
            case 4:
                switch (keyULong)
                {
                    case 0x00000000001C7FC1UL:
                        return ref _war;
                    case 0x0000000000097F1DUL:
                        return ref camp;
                }
                break;
            case 5:
                switch (keyULong)
                {
                    case 0x000000000412F391UL:
                        return ref _help;
                    case 0x0000000004163EDCUL:
                        return ref _load;
                    case 0x000000000416D131UL:
                        return ref _menu;
                    case 0x0000000004178175UL:
                        return ref _name;
                    case 0x00000000041B6013UL:
                        return ref _save;
                    case 0x00000000041BAF56UL:
                        return ref _spot;
                    case 0x00000000041C6FD2UL:
                        return ref _tool;
                    case 0x00000000041E76E7UL:
                        return ref _war2;
                    case 0x00000000014B5162UL:
                        return ref bloom;
                }
                break;
            case 6:
                switch (keyULong)
                {
                    case 0x0000000096361214UL:
                        return ref _corps;
                    case 0x00000000964DFC8DUL:
                        return ref _diplo;
                    case 0x0000000096BD33F7UL:
                        return ref _help2;
                    case 0x00000000973715CEUL:
                        return ref _load2;
                    case 0x00000000974C3C17UL:
                        return ref _menu2;
                    case 0x00000000974C4EEDUL:
                        return ref _merce;
                    case 0x000000009765B5EBUL:
                        return ref _name2;
                    case 0x0000000097F4E2C1UL:
                        return ref _save2;
                    case 0x0000000098005770UL:
                        return ref _spot2;
                    case 0x00000000981C295CUL:
                        return ref _tool2;
                    case 0x0000000037D4068DUL:
                        return ref discus;
                    case 0x0000000040C04B28UL:
                        return ref font10;
                    case 0x0000000040C04B2AUL:
                        return ref font12;
                    case 0x0000000040C04B2CUL:
                        return ref font14;
                    case 0x0000000040C04B2EUL:
                        return ref font16;
                    case 0x0000000040C04B4DUL:
                        return ref font20;
                    case 0x000000005C8FE96EUL:
                        return ref member;
                }
                break;
            case 7:
                switch (keyULong)
                {
                    case 0x00000015B01FC50AUL:
                        return ref _battle;
                    case 0x00000015B5D09CE6UL:
                        return ref _corps2;
                    case 0x00000015B8D5F875UL:
                        return ref _detail;
                    case 0x00000015B939E87EUL:
                        return ref _dialog;
                    case 0x00000015B9458063UL:
                        return ref _diplo2;
                    case 0x00000015DE076843UL:
                        return ref _merce2;
                    case 0x00000015F80CA332UL:
                        return ref _spot22;
                    case 0x00000015F8743906UL:
                        return ref _status;
                }
                break;
            case 8:
                switch (keyULong)
                {
                    case 0x0000032274977A74UL:
                        return ref _battle2;
                    case 0x00000322BEEEDB39UL:
                        return ref _bstatus;
                    case 0x000003234695173BUL:
                        return ref _commenu;
                    case 0x00000323B6ECE8EBUL:
                        return ref _detail2;
                    case 0x00000323C55E9A38UL:
                        return ref _dialog2;
                    case 0x00000329173AF520UL:
                        return ref _message;
                    case 0x0000032CE8CC3DE0UL:
                        return ref _status2;
                    case 0x00000104CFFCEB0AUL:
                        return ref btl_auto;
                    case 0x00000167EC769431UL:
                        return ref gain_per;
                    case 0x000001DF0E02DEB1UL:
                        return ref loyal_up;
                    case 0x000001ECCA2E2BEDUL:
                        return ref max_unit;
                    case 0x000001FB2BD0B92EUL:
                        return ref my_range;
                    case 0x0000020BAD04C474UL:
                        return ref npm_play;
                    case 0x0000023742B809ABUL:
                        return ref politics;
                    case 0x0000025B0D88AF53UL:
                        return ref raid_bdr;
                    case 0x0000025B0D88B636UL:
                        return ref raid_coe;
                    case 0x0000025B0D88E9BDUL:
                        return ref raid_max;
                    case 0x0000025B0D88EADBUL:
                        return ref raid_min;
                    case 0x0000025FE9F85C3EUL:
                        return ref ring_red;
                    case 0x000002919D58EDFAUL:
                        return ref trust_up;
                    case 0x000002CE700BF0C4UL:
                        return ref win_gain;
                }
                break;
            case 9:
                switch (keyULong)
                {
                    case 0x000074059885AF3FUL:
                        return ref _bstatus2;
                    case 0x00007419338C5B89UL:
                        return ref _commenu2;
                    case 0x00007481D312BADBUL:
                        return ref _heromenu;
                    case 0x000074F05B856DA2UL:
                        return ref _message2;
                    case 0x000075738D866180UL:
                        return ref _scenario;
                    case 0x0000210D16BE1074UL:
                        return ref actor_per;
                    case 0x000025B210BF1E41UL:
                        return ref btl_limit;
                    case 0x000028752C0F172EUL:
                        return ref color_red;
                    case 0x0000320BE3CB61B2UL:
                        return ref font_face;
                    case 0x0000320BE3CB8DC7UL:
                        return ref font_file;
                    case 0x0000320BE3D64C44UL:
                        return ref font_text;
                    case 0x000032AE2DC6EE9AUL:
                        return ref fv_hp_per;
                    case 0x000032AE427174C3UL:
                        return ref fv_mp_per;
                    case 0x000048625D21C80BUL:
                        return ref mode_easy;
                    case 0x000048625D241968UL:
                        return ref mode_hard;
                    case 0x000048625D279B39UL:
                        return ref mode_luna;
                    case 0x00004A6705963C2BUL:
                        return ref narabijun;
                    case 0x000057DCD0D91F18UL:
                        return ref ring_blue;
                    case 0x000057DCD0DA279FUL:
                        return ref ring_cyan;
                    case 0x00005E4418E30620UL:
                        return ref title_bgm;
                    case 0x0000683EBBB3852FUL:
                        return ref wnd_alpha;
                    case 0x0000685CF578D28EUL:
                        return ref world_bgm;
                }
                break;
            case 10:
                switch (keyULong)
                {
                    case 0x0010D6C381B501A9UL:
                        return ref _heromenu2;
                    case 0x0010F131DE1EE4E2UL:
                        return ref _powerinfo;
                    case 0x0010F9B3746C1782UL:
                        return ref _scenario2;
                    case 0x000536B69E0C02B3UL:
                        return ref battle_bgm;
                    case 0x0005494B1ED50257UL:
                        return ref bgm_volume;
                    case 0x000572BC5A9B57ECUL:
                        return ref btl_healer;
                    case 0x0005AD0A8DEFC5B9UL:
                        return ref cavalry_ai;
                    case 0x0005D8EF5E2223C8UL:
                        return ref color_blue;
                    case 0x0005D8EF5E232C4FUL:
                        return ref color_cyan;
                    case 0x0005D8EF5E2B2E29UL:
                        return ref color_name;
                    case 0x0005D8EF5E2FE861UL:
                        return ref color_text;
                    case 0x000622769DBB114EUL:
                        return ref damage_max;
                    case 0x000622769DBB126CUL:
                        return ref damage_min;
                    case 0x00063C4E23450A79UL:
                        return ref diplo_down;
                    case 0x00073BB7EDFC06CAUL:
                        return ref font_title;
                    case 0x00073C05D58D6262UL:
                        return ref force_fire;
                    case 0x0007447D7AA41397UL:
                        return ref free_raise;
                    case 0x000A01D1ED4BBC3AUL:
                        return ref loyal_down;
                    case 0x000C632A52F23E3EUL:
                        return ref quota_view;
                    case 0x000C98675BB1D9B4UL:
                        return ref race_label;
                    case 0x000CB2EA2F44CB43UL:
                        return ref ring_alpha;
                    case 0x000CB2EA2FF4C8E2UL:
                        return ref ring_green;
                    case 0x000CC4F94E76DD07UL:
                        return ref roam_avoid;
                    case 0x000D1C5704722E6DUL:
                        return ref senior_bdr;
                    case 0x000D3F97D23EA689UL:
                        return ref spot_fire1;
                    case 0x000D3F97D23EA68AUL:
                        return ref spot_fire2;
                    case 0x000D3F97D39A8746UL:
                        return ref spot_route;
                    case 0x000D9FD798D908F4UL:
                        return ref title_name;
                    case 0x000F1111261914D6UL:
                        return ref wnd_button;
                    case 0x000F219A60B473C1UL:
                        return ref wskin_cell;
                    case 0x001077AC78A14F1FUL:
                        return ref zone_alpha;
                }
                break;
            case 11:
                switch (keyULong)
                {
                    case 0x0272BBDCC6D02A15UL:
                        return ref _personinfo;
                    case 0x0272DC351A7714ACUL:
                        return ref _powerinfo2;
                    case 0x00B771999AEDD6C4UL:
                        return ref arbeit_gain;
                    case 0x00B771999AF84F3EUL:
                        return ref arbeit_turn;
                    case 0x00C0E864D7BF5C38UL:
                        return ref battle_fast;
                    case 0x00C9953647612A80UL:
                        return ref btl_counter;
                    case 0x00C9953B8E08CED9UL:
                        return ref btl_limit_c;
                    case 0x00C9953C27131F6FUL:
                        return ref btl_minimap;
                    case 0x00C9953C4FA869F6UL:
                        return ref btl_msgtime;
                    case 0x00C9953FC934304CUL:
                        return ref btl_slowper;
                    case 0x00C9953FE8AD746DUL:
                        return ref btl_standby;
                    case 0x00C9954102A62FB2UL:
                        return ref btl_unitmax;
                    case 0x00C99542205C5B97UL:
                        return ref btl_wingmax;
                    case 0x00D85A989B827652UL:
                        return ref color_green;
                    case 0x00D85A989D446246UL:
                        return ref color_white;
                    case 0x00D85DD641807312UL:
                        return ref compati_bad;
                    case 0x00E2FB24CC098048UL:
                        return ref damage_max2;
                    case 0x00E2FB24CC09A99EUL:
                        return ref damage_min2;
                    case 0x010BA19522D8AFF4UL:
                        return ref font_detail;
                    case 0x010F05E0A1F765EBUL:
                        return ref fv_move_per;
                    case 0x0141135F2B2CEE4DUL:
                        return ref item_window;
                    case 0x0183160430690CFCUL:
                        return ref mode_normal;
                    case 0x018FC4FC014B33D3UL:
                        return ref neutral_max;
                    case 0x018FC4FC014B34F1UL:
                        return ref neutral_min;
                    case 0x01B692D6D9A99993UL:
                        return ref power_order;
                    case 0x01D206F05FE2422AUL:
                        return ref race_suffix;
                    case 0x01D5DBD90F6EF70AUL:
                        return ref ring_orange;
                    case 0x01D5DBD913F40BCAUL:
                        return ref ring_purple;
                    case 0x01D5DBD93758B626UL:
                        return ref ring_yellow;
                    case 0x01E51893A481B6B3UL:
                        return ref senior_cost;
                    case 0x01E9CD572AB94D48UL:
                        return ref sound_count;
                    case 0x01F44E6C8838B52BUL:
                        return ref talent_mode;
                    case 0x01F81A29160BC670UL:
                        return ref title_bgm_s;
                    case 0x020B5C2433D9FD9FUL:
                        return ref unit_vanish;
                    case 0x02614BEDB4506742UL:
                        return ref zone_radius;
                }
                break;
            case 12:
                switch (keyULong)
                {
                    case 0x5A9526E8BC16150BUL:
                        return ref _personinfo2;
                    case 0x5A99D3ACFB8E17F8UL:
                        return ref _powerselect;
                    case 0x1D2291B5119A3C5CUL:
                        return ref btl_mode_coe;
                    case 0x1D22921B4CF9F0DAUL:
                        return ref btl_raidback;
                    case 0x1D229233DC2A00A4UL:
                        return ref btl_semiauto;
                    case 0x1F45180E9AE9083AUL:
                        return ref color_orange;
                    case 0x1F45180EC2D2C756UL:
                        return ref color_yellow;
                    case 0x210D22C97A3455DAUL:
                        return ref dead_penalty;
                    case 0x21587DDAA4C09C1BUL:
                        return ref diplo_frd_up;
                    case 0x21587DDADADB0A1BUL:
                        return ref diplo_sup_up;
                    case 0x2414EA835CB0BAF6UL:
                        return ref employ_range;
                    case 0x26AE5A8CC6111AF1UL:
                        return ref font_balloon;
                    case 0x26B001E33B46CFD1UL:
                        return ref foreign_test;
                    case 0x272BC9920C52B96FUL:
                        return ref fv_hprec_per;
                    case 0x272BD5688F378901UL:
                        return ref fv_level_per;
                    case 0x272BECAE3958B7D6UL:
                        return ref fv_speed_per;
                    case 0x34CE137DC1C1F69BUL:
                        return ref leave_period;
                    case 0x3583BB9DF1B94BD6UL:
                        return ref loyal_border;
                    case 0x374DDBC3D0204922UL:
                        return ref merits_bonus;
                    case 0x37FA7CF785A6700AUL:
                        return ref movement_num;
                    case 0x389EDBEF16230D03UL:
                        return ref myhelp_range;
                    case 0x3EF3ACED2648544FUL:
                        return ref picture_fade;
                    case 0x4237D1BDA4E2A7CAUL:
                        return ref quarter_view;
                    case 0x4680C7FA7A9DBD2DUL:
                        return ref skill_offset;
                    case 0x46CAAD997B4958ECUL:
                        return ref sound_volume;
                    case 0x472EE6363A04BEBBUL:
                        return ref support_gain;
                    case 0x48DBC7F08341A799UL:
                        return ref title_volume;
                    case 0x4976163BD4441FA1UL:
                        return ref trust_morale;
                    case 0x4BA419D8C1B22332UL:
                        return ref unicode_test;
                    case 0x4BA4513B2F05DEBEUL:
                        return ref unit_use_coe;
                    case 0x4FBAA5928BD7F723UL:
                        return ref war_capacity;
                    case 0x50A9A12636E3C88FUL:
                        return ref world_volume;
                }
                break;
            case 13:
                switch (keyULong)
                {
                    case 0x5A9526E8BD2D385EUL when key[StringHashUtility.HashLengthMax] == 't':
                        return ref _personselect;
                    case 0x5A99D3ACFB8E17F8UL when key[StringHashUtility.HashLengthMax] == '2':
                        return ref _powerselect2;
                    case 0x5AC750A9960A48BCUL when key[StringHashUtility.HashLengthMax] == 't':
                        return ref _scenariotext;
                    case 0x1A836B336569C229UL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref arbeit_player;
                    case 0x1D9D3D21A41A08B2UL when key[StringHashUtility.HashLengthMax] == 't':
                        return ref b_sound_count;
                    case 0x1BE19693307C900AUL when key[StringHashUtility.HashLengthMax] == 'e':
                        return ref battle_volume;
                    case 0x1CCCDE8A6E522FCBUL when key[StringHashUtility.HashLengthMax] == 'h':
                        return ref bold_spotpath;
                    case 0x1D22915C51411E83UL when key[StringHashUtility.HashLengthMax] == 't':
                        return ref btl_intercept;
                    case 0x1D22919B8AF471E3UL when key[StringHashUtility.HashLengthMax] == 't':
                        return ref btl_lineshift;
                    case 0x1D2291B1A5C38B2FUL when key[StringHashUtility.HashLengthMax] == 'w':
                        return ref btl_minimap_w;
                    case 0x1F45180E6C0C8709UL when key[StringHashUtility.HashLengthMax] == '1':
                        return ref color_detail1;
                    case 0x1F45180E6C0C8709UL when key[StringHashUtility.HashLengthMax] == '2':
                        return ref color_detail2;
                    case 0x1F45180E6C0C8709UL when key[StringHashUtility.HashLengthMax] == '3':
                        return ref color_detail3;
                    case 0x1F45180E90C31EAEUL when key[StringHashUtility.HashLengthMax] == 'a':
                        return ref color_magenta;
                    case 0x20CE4C517C4C3179UL when key[StringHashUtility.HashLengthMax] == 'i':
                        return ref damage_consti;
                    case 0x24A99D949BFBB13BUL when key[StringHashUtility.HashLengthMax] == 'e':
                        return ref event_bg_size;
                    case 0x24CBA5464C30FE33UL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref executive_bdr;
                    case 0x25B9EB17735E4C61UL when key[StringHashUtility.HashLengthMax] == 'y':
                        return ref face_colorkey;
                    case 0x26AE5A9569ACF7C6UL when key[StringHashUtility.HashLengthMax] == 'e':
                        return ref font_prologue;
                    case 0x272BB38EE67E5E47UL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref fv_attack_per;
                    case 0x272BB980B97C4CF4UL when key[StringHashUtility.HashLengthMax] == 'l':
                        return ref fv_consti_mul;
                    case 0x272BED21A6F0C827UL when key[StringHashUtility.HashLengthMax] == 'l':
                        return ref fv_summon_mul;
                    case 0x2E67CCBEC90BBEE6UL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref item_sell_per;
                    case 0x39C857D5CD954FF9UL when key[StringHashUtility.HashLengthMax] == 'g':
                        return ref newpower_flag;
                    case 0x3A719AD669AF6D5EUL when key[StringHashUtility.HashLengthMax] == 'n':
                        return ref notalent_down;
                    case 0x3EF3ACED264890C3UL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref picture_floor;
                    case 0x3EF3ACED265380E8UL when key[StringHashUtility.HashLengthMax] == 's':
                        return ref picture_trans;
                    case 0x4680C7EF89526694UL when key[StringHashUtility.HashLengthMax] == 'd':
                        return ref skillicon_add;
                    case 0x4680C7EF8952669CUL when key[StringHashUtility.HashLengthMax] == 'l':
                        return ref skillicon_all;
                    case 0x46D912E164F20DD2UL when key[StringHashUtility.HashLengthMax] == 'y':
                        return ref spot_capacity;
                    case 0x472EE6363A0D3FECUL when key[StringHashUtility.HashLengthMax] == 'e':
                        return ref support_range;
                    case 0x4BA45139BC6B2835UL when key[StringHashUtility.HashLengthMax] == 'm':
                        return ref unit_scream_m;
                    case 0x4BA45139BC6B2835UL when key[StringHashUtility.HashLengthMax] == 'n':
                        return ref unit_scream_n;
                    case 0x4BA45139E156C3CCUL when key[StringHashUtility.HashLengthMax] == 'r':
                        return ref unit_slow_per;
                }
                break;
            case 14:
                switch (keyULong)
                {
                    case 0x5A9526E8BD2D385EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("t2"):
                        return ref _personselect2;
                    case 0x5AC750A9960A48BCUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("t2"):
                        return ref _scenariotext2;
                    case 0x1D9D3D21A428B6D3UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("me"):
                        return ref b_sound_volume;
                    case 0x1CCCDE8A6E521A55UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("el"):
                        return ref bold_spotlabel;
                    case 0x1D2290AF134AAB09UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("de"):
                        return ref btl_attackmode;
                    case 0x1D2290B9D62AA7BDUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("de"):
                        return ref btl_battlemode;
                    case 0x1D2290CFEC6A0BF8UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ot"):
                        return ref btl_castle_lot;
                    case 0x1D2290D3D0483D47UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("de"):
                        return ref btl_chargemode;
                    case 0x1D2291B1A7BF56D3UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ge"):
                        return ref btl_min_damage;
                    case 0x1E5A23AC1E33ADF6UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("st"):
                        return ref caution_adjust;
                    case 0x1F45180E94E9DBC4UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("lp"):
                        return ref color_namehelp;
                    case 0x1F45180E9F15EB9DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ue"):
                        return ref color_prologue;
                    case 0x210F67ACA811C6DCUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ng"):
                        return ref default_ending;
                    case 0x21587DDA980FF42BUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("er"):
                        return ref diplo_cost_per;
                    case 0x21587DDAA4C099A5UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("wn"):
                        return ref diplo_frd_down;
                    case 0x21587DDAC159184CUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("tv"):
                        return ref diplo_mode_itv;
                    case 0x24CBA5464C30FE63UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("st"):
                        return ref executive_cost;
                    case 0x26AE5A972C388263UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("el"):
                        return ref font_spotlabel;
                    case 0x26AE5A9798B0EF56UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ze"):
                        return ref font_text_size;
                    case 0x2713D2C22179C454UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ak"):
                        return ref fullbody_speak;
                    case 0x37F22E95651AED42UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("xt"):
                        return ref mode_easy_text;
                    case 0x37F22E972FB7BB8BUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("xt"):
                        return ref mode_hard_text;
                    case 0x37F22E99E5A4A1F8UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("xt"):
                        return ref mode_luna_text;
                    case 0x3EF3ACED2646199DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("er"):
                        return ref picture_center;
                    case 0x435DF0C843FDB663UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("dr"):
                        return ref raise_yabo_bdr;
                    case 0x4680C7EF895267CCUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("em"):
                        return ref skillicon_item;
                    case 0x4A0EDD8CC634AD1DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("me"):
                        return ref t_sound_volume;
                    case 0x4852207ED808E97DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("oe"):
                        return ref target_way_coe;
                    case 0x48DBC7F05CF24A81UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("op"):
                        return ref title_menu_top;
                    case 0x4976163BAF155330UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("er"):
                        return ref trust_down_per;
                    case 0x4BA451310297C5D9UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ul"):
                        return ref unit_drain_mul;
                    case 0x4BA45134FBD59DD4UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("rm"):
                        return ref unit_keep_form;
                    case 0x4BA4513596A0D8EFUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ax"):
                        return ref unit_level_max;
                    case 0x4BA4513A00D4CC1EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("hp"):
                        return ref unit_status_hp;
                }
                break;
            case 15:
                switch (keyULong)
                {
                    case 0x1D9D3D21A41EC6CFUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("max"):
                        return ref b_sound_itv_max;
                    case 0x1D9D3D21A41EC6CFUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("min"):
                        return ref b_sound_itv_min;
                    case 0x1CCCDE821C505B29UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ath"):
                        return ref bold_escapepath;
                    case 0x1D2290C03A3AD892UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("pha"):
                        return ref btl_blind_alpha;
                    case 0x1D2291F930D13977UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("max"):
                        return ref btl_prepare_max;
                    case 0x1D2291F930D13977UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("min"):
                        return ref btl_prepare_min;
                    case 0x1D22921DDFBB4C6FUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("coe"):
                        return ref btl_retreat_coe;
                    case 0x1F45180EAB43348DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("bel"):
                        return ref color_spotlabel;
                    case 0x21587DDAA4C09B57UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ace"):
                        return ref diplo_frd_peace;
                    case 0x21587DDABC16BC46UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_up"):
                        return ref diplo_league_up;
                    case 0x2713D2C221797295UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ail"):
                        return ref fullbody_detail;
                    case 0x4BA4512EFBEDEE6CUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("bdr"):
                        return ref unit_action_bdr;
                    case 0x4BA451319FDACC03UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("run"):
                        return ref unit_escape_run;
                    case 0x4BA45133217BABF0UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("nge"):
                        return ref unit_hand_range;
                    case 0x4BA45134FBD59D65UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("lor"):
                        return ref unit_keep_color;
                    case 0x4BA4513822448476UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("per"):
                        return ref unit_poison_per;
                    case 0x4BA45139C2D34C10UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("bdr"):
                        return ref unit_search_bdr;
                    case 0x4BA4513A00D4CC1EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("min"):
                        return ref unit_status_min;
                    case 0x4BA4513A00D4CC1EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("rec"):
                        return ref unit_status_rec;
                    case 0x4BA4513A05294311UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("mit"):
                        return ref unit_suck_limit;
                    case 0x4BA4513A06488865UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("min"):
                        return ref unit_summon_min;
                    case 0x4BA4513B9E9C1C84UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("nge"):
                        return ref unit_view_range;
                    case 0x4FBB45C0742A393FUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("lay"):
                        return ref watching_replay;
                }
                break;
            case 16:
                switch (keyULong)
                {
                    case 0x1A836B3364600A78UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("lose"):
                        return ref arbeit_gain_lose;
                    case 0x1A836B33656E8CF1UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_coe"):
                        return ref arbeit_price_coe;
                    case 0x1D2290C3BDDD6408UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("idth"):
                        return ref btl_breast_width;
                    case 0x1D22912858710A53UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ough"):
                        return ref btl_gate_through;
                    case 0x1D2291CB27616662UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_bdr"):
                        return ref btl_nocastle_bdr;
                    case 0x1D22921DCE843C88UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("line"):
                        return ref btl_replace_line;
                    case 0x1EE1360EF6802B2FUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_per"):
                        return ref circle_alpha_per;
                    case 0x1F45180EAB43348DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("bel2"):
                        return ref color_spotlabel2;
                    case 0x21587DDA92CFDCE5UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("iend"):
                        return ref diplo_bestfriend;
                    case 0x21587DDA948D8E5AUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_per"):
                        return ref diplo_budget_per;
                    case 0x21587DDAC165007DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ower"):
                        return ref diplo_most_power;
                    case 0x2414EA835C849170UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_coe"):
                        return ref employ_price_coe;
                    case 0x3583BB9DFE8659A5UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_bdr"):
                        return ref loyal_escape_bdr;
                    case 0x37F22E9AFF2EE090UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("text"):
                        return ref mode_normal_text;
                    case 0x435DF0C81402100DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ower"):
                        return ref raise_most_power;
                    case 0x45F6498AD80D0307UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ect2"):
                        return ref scenario_select2;
                    case 0x4680C7EF895266A3UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("sist"):
                        return ref skillicon_assist;
                    case 0x4680C7EF8952682CUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ader"):
                        return ref skillicon_leader;
                    case 0x48DBC7F05CF24A7FUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ight"):
                        return ref title_menu_right;
                    case 0x48DBC7F05CF24A80UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("pace"):
                        return ref title_menu_space;
                    case 0x496CB9BCD4EC5D87UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("rage"):
                        return ref training_average;
                    case 0x4BA4512F8CA008B2UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("cram"):
                        return ref unit_battle_cram;
                    case 0x4BA451302571508AUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("cram"):
                        return ref unit_castle_cram;
                    case 0x4BA45133EA90E7C7UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ight"):
                        return ref unit_image_right;
                    case 0x4BA45135221C29FFUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("peed"):
                        return ref unit_knock_speed;
                    case 0x4BA4513A0EC99448UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("lide"):
                        return ref unit_sword_slide;
                    case 0x4D42EAEF45A80A9CUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_coe"):
                        return ref vassal_price_coe;
                }
                break;
            case 17:
                switch (keyULong)
                {
                    case 0x1A836B3365168BA2UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_mini"):
                        return ref arbeit_money_mini;
                    case 0x1A836B33660D36B0UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("l_coe"):
                        return ref arbeit_vassal_coe;
                    case 0x1D9D3D21A4228A13UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("epeat"):
                        return ref b_sound_no_repeat;
                    case 0x21587DDADAD07B3DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("s_frd"):
                        return ref diplo_success_frd;
                    case 0x21587DDADAD07B3DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("s_leg"):
                        return ref diplo_success_leg;
                    case 0x21587DDADAD07B3DUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("s_sup"):
                        return ref diplo_success_sup;
                    case 0x4680C7EF8952693AUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("ecial"):
                        return ref skillicon_special;
                    case 0x46D912E2DFC03319UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("route"):
                        return ref spot_escape_route;
                    case 0x472EE6363A04BEBBUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_lose"):
                        return ref support_gain_lose;
                    case 0x472EE6363A0F279EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("t_bdr"):
                        return ref support_trust_bdr;
                    case 0x4BA4512F422B470EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("range"):
                        return ref unit_attack_range;
                    case 0x4BA45130405BA4F4UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("slide"):
                        return ref unit_charge_slide;
                    case 0x4BA45131832E4EE1UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_heal"):
                        return ref unit_element_heal;
                    case 0x4BA451319FDACC03UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("range"):
                        return ref unit_escape_range;
                    case 0x4BA4513A00D4CC1EUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("death"):
                        return ref unit_status_death;
                    case 0x4BA4513A06488865UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("level"):
                        return ref unit_summon_level;
                }
                break;
            case 18:
                switch (keyULong)
                {
                    case 0x1F458FF777A01925UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("al_bdr"):
                        return ref compati_vassal_bdr;
                    case 0x21587DDAA4C09BFBUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("rndown"):
                        return ref diplo_frd_turndown;
                    case 0x21587DDADADB09FBUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("rndown"):
                        return ref diplo_sup_turndown;
                    case 0x39C7786C2FDE9157UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("er_max"):
                        return ref neutral_member_max;
                    case 0x39C7786C2FDE9157UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("er_min"):
                        return ref neutral_member_min;
                    case 0x4976163BAF155333UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("upport"):
                        return ref trust_down_support;
                    case 0x4BA45133E7B479A4UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("n_miss"):
                        return ref unit_illusion_miss;
                    case 0x4BA451363FCC1A11UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_slide"):
                        return ref unit_missile_slide;
                    case 0x4BA451392C04CED2UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_speed"):
                        return ref unit_retreat_speed;
                }
                break;
            case 19:
                switch (keyULong)
                {
                    case 0x4BA451305EB52B7AUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("on_miss"):
                        return ref unit_confusion_miss;
                    case 0x4BA45135A55F7BFCUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_height"):
                        return ref unit_lifebar_height;
                    case 0x4BA451392C04CED2UL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("_damage"):
                        return ref unit_retreat_damage;
                    case 0x4BA45139F0B15E0CUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("use_per"):
                        return ref unit_specialuse_per;
                }
                break;
            case 21:
                switch (keyULong)
                {
                    case 0x4BA451302571508AUL when key.Slice(StringHashUtility.HashLengthMax).SequenceEqual("forcefire"):
                        return ref unit_castle_forcefire;
                }
                break;
        }

    FALSE:
        return ref Unsafe.NullRef<Pair_NullableString_NullableInt_ArrayElement?>();
    }

    public void IncrementToken(uint indexEqualToOrGreaterThan, uint count)
    {
        if (Kind >= indexEqualToOrGreaterThan)
        {
            Kind += count;
        }

        if (BracketLeft >= indexEqualToOrGreaterThan)
        {
            BracketLeft += count;
        }

        if (BracketRight >= indexEqualToOrGreaterThan)
        {
            BracketRight += count;
        }
        _war?.IncrementToken(indexEqualToOrGreaterThan, count);
        camp?.IncrementToken(indexEqualToOrGreaterThan, count);
        _help?.IncrementToken(indexEqualToOrGreaterThan, count);
        _load?.IncrementToken(indexEqualToOrGreaterThan, count);
        _menu?.IncrementToken(indexEqualToOrGreaterThan, count);
        _name?.IncrementToken(indexEqualToOrGreaterThan, count);
        _save?.IncrementToken(indexEqualToOrGreaterThan, count);
        _spot?.IncrementToken(indexEqualToOrGreaterThan, count);
        _tool?.IncrementToken(indexEqualToOrGreaterThan, count);
        _war2?.IncrementToken(indexEqualToOrGreaterThan, count);
        bloom?.IncrementToken(indexEqualToOrGreaterThan, count);
        _corps?.IncrementToken(indexEqualToOrGreaterThan, count);
        _diplo?.IncrementToken(indexEqualToOrGreaterThan, count);
        _help2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _load2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _menu2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _merce?.IncrementToken(indexEqualToOrGreaterThan, count);
        _name2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _save2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _spot2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _tool2?.IncrementToken(indexEqualToOrGreaterThan, count);
        discus?.IncrementToken(indexEqualToOrGreaterThan, count);
        font10?.IncrementToken(indexEqualToOrGreaterThan, count);
        font12?.IncrementToken(indexEqualToOrGreaterThan, count);
        font14?.IncrementToken(indexEqualToOrGreaterThan, count);
        font16?.IncrementToken(indexEqualToOrGreaterThan, count);
        font20?.IncrementToken(indexEqualToOrGreaterThan, count);
        member?.IncrementToken(indexEqualToOrGreaterThan, count);
        _battle?.IncrementToken(indexEqualToOrGreaterThan, count);
        _corps2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _detail?.IncrementToken(indexEqualToOrGreaterThan, count);
        _dialog?.IncrementToken(indexEqualToOrGreaterThan, count);
        _diplo2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _merce2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _spot22?.IncrementToken(indexEqualToOrGreaterThan, count);
        _status?.IncrementToken(indexEqualToOrGreaterThan, count);
        _battle2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _bstatus?.IncrementToken(indexEqualToOrGreaterThan, count);
        _commenu?.IncrementToken(indexEqualToOrGreaterThan, count);
        _detail2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _dialog2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _message?.IncrementToken(indexEqualToOrGreaterThan, count);
        _status2?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_auto?.IncrementToken(indexEqualToOrGreaterThan, count);
        gain_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        loyal_up?.IncrementToken(indexEqualToOrGreaterThan, count);
        max_unit?.IncrementToken(indexEqualToOrGreaterThan, count);
        my_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        npm_play?.IncrementToken(indexEqualToOrGreaterThan, count);
        politics?.IncrementToken(indexEqualToOrGreaterThan, count);
        raid_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        raid_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        raid_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        raid_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_red?.IncrementToken(indexEqualToOrGreaterThan, count);
        trust_up?.IncrementToken(indexEqualToOrGreaterThan, count);
        win_gain?.IncrementToken(indexEqualToOrGreaterThan, count);
        _bstatus2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _commenu2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _heromenu?.IncrementToken(indexEqualToOrGreaterThan, count);
        _message2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _scenario?.IncrementToken(indexEqualToOrGreaterThan, count);
        actor_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_limit?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_red?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_face?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_file?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_hp_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_mp_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_easy?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_hard?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_luna?.IncrementToken(indexEqualToOrGreaterThan, count);
        narabijun?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_blue?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_cyan?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_bgm?.IncrementToken(indexEqualToOrGreaterThan, count);
        wnd_alpha?.IncrementToken(indexEqualToOrGreaterThan, count);
        world_bgm?.IncrementToken(indexEqualToOrGreaterThan, count);
        _heromenu2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _powerinfo?.IncrementToken(indexEqualToOrGreaterThan, count);
        _scenario2?.IncrementToken(indexEqualToOrGreaterThan, count);
        battle_bgm?.IncrementToken(indexEqualToOrGreaterThan, count);
        bgm_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_healer?.IncrementToken(indexEqualToOrGreaterThan, count);
        cavalry_ai?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_blue?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_cyan?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_name?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        damage_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        damage_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_down?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_title?.IncrementToken(indexEqualToOrGreaterThan, count);
        force_fire?.IncrementToken(indexEqualToOrGreaterThan, count);
        free_raise?.IncrementToken(indexEqualToOrGreaterThan, count);
        loyal_down?.IncrementToken(indexEqualToOrGreaterThan, count);
        quota_view?.IncrementToken(indexEqualToOrGreaterThan, count);
        race_label?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_alpha?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_green?.IncrementToken(indexEqualToOrGreaterThan, count);
        roam_avoid?.IncrementToken(indexEqualToOrGreaterThan, count);
        senior_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        spot_fire1?.IncrementToken(indexEqualToOrGreaterThan, count);
        spot_fire2?.IncrementToken(indexEqualToOrGreaterThan, count);
        spot_route?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_name?.IncrementToken(indexEqualToOrGreaterThan, count);
        wnd_button?.IncrementToken(indexEqualToOrGreaterThan, count);
        wskin_cell?.IncrementToken(indexEqualToOrGreaterThan, count);
        zone_alpha?.IncrementToken(indexEqualToOrGreaterThan, count);
        _personinfo?.IncrementToken(indexEqualToOrGreaterThan, count);
        _powerinfo2?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_gain?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_turn?.IncrementToken(indexEqualToOrGreaterThan, count);
        battle_fast?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_counter?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_limit_c?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_minimap?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_msgtime?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_slowper?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_standby?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_unitmax?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_wingmax?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_green?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_white?.IncrementToken(indexEqualToOrGreaterThan, count);
        compati_bad?.IncrementToken(indexEqualToOrGreaterThan, count);
        damage_max2?.IncrementToken(indexEqualToOrGreaterThan, count);
        damage_min2?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_detail?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_move_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        item_window?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_normal?.IncrementToken(indexEqualToOrGreaterThan, count);
        neutral_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        neutral_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        power_order?.IncrementToken(indexEqualToOrGreaterThan, count);
        race_suffix?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_orange?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_purple?.IncrementToken(indexEqualToOrGreaterThan, count);
        ring_yellow?.IncrementToken(indexEqualToOrGreaterThan, count);
        senior_cost?.IncrementToken(indexEqualToOrGreaterThan, count);
        sound_count?.IncrementToken(indexEqualToOrGreaterThan, count);
        talent_mode?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_bgm_s?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_vanish?.IncrementToken(indexEqualToOrGreaterThan, count);
        zone_radius?.IncrementToken(indexEqualToOrGreaterThan, count);
        _personinfo2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _powerselect?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_mode_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_raidback?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_semiauto?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_orange?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_yellow?.IncrementToken(indexEqualToOrGreaterThan, count);
        dead_penalty?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_up?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_sup_up?.IncrementToken(indexEqualToOrGreaterThan, count);
        employ_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_balloon?.IncrementToken(indexEqualToOrGreaterThan, count);
        foreign_test?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_hprec_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_level_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_speed_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        leave_period?.IncrementToken(indexEqualToOrGreaterThan, count);
        loyal_border?.IncrementToken(indexEqualToOrGreaterThan, count);
        merits_bonus?.IncrementToken(indexEqualToOrGreaterThan, count);
        movement_num?.IncrementToken(indexEqualToOrGreaterThan, count);
        myhelp_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        picture_fade?.IncrementToken(indexEqualToOrGreaterThan, count);
        quarter_view?.IncrementToken(indexEqualToOrGreaterThan, count);
        skill_offset?.IncrementToken(indexEqualToOrGreaterThan, count);
        sound_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        support_gain?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        trust_morale?.IncrementToken(indexEqualToOrGreaterThan, count);
        unicode_test?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_use_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        war_capacity?.IncrementToken(indexEqualToOrGreaterThan, count);
        world_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        _personselect?.IncrementToken(indexEqualToOrGreaterThan, count);
        _powerselect2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _scenariotext?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_player?.IncrementToken(indexEqualToOrGreaterThan, count);
        b_sound_count?.IncrementToken(indexEqualToOrGreaterThan, count);
        battle_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        bold_spotpath?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_intercept?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_lineshift?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_minimap_w?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_detail1?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_detail2?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_detail3?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_magenta?.IncrementToken(indexEqualToOrGreaterThan, count);
        damage_consti?.IncrementToken(indexEqualToOrGreaterThan, count);
        event_bg_size?.IncrementToken(indexEqualToOrGreaterThan, count);
        executive_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        face_colorkey?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_prologue?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_attack_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_consti_mul?.IncrementToken(indexEqualToOrGreaterThan, count);
        fv_summon_mul?.IncrementToken(indexEqualToOrGreaterThan, count);
        item_sell_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        newpower_flag?.IncrementToken(indexEqualToOrGreaterThan, count);
        notalent_down?.IncrementToken(indexEqualToOrGreaterThan, count);
        picture_floor?.IncrementToken(indexEqualToOrGreaterThan, count);
        picture_trans?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_add?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_all?.IncrementToken(indexEqualToOrGreaterThan, count);
        spot_capacity?.IncrementToken(indexEqualToOrGreaterThan, count);
        support_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_scream_m?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_scream_n?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_slow_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        _personselect2?.IncrementToken(indexEqualToOrGreaterThan, count);
        _scenariotext2?.IncrementToken(indexEqualToOrGreaterThan, count);
        b_sound_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        bold_spotlabel?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_attackmode?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_battlemode?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_castle_lot?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_chargemode?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_min_damage?.IncrementToken(indexEqualToOrGreaterThan, count);
        caution_adjust?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_namehelp?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_prologue?.IncrementToken(indexEqualToOrGreaterThan, count);
        default_ending?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_cost_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_down?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_mode_itv?.IncrementToken(indexEqualToOrGreaterThan, count);
        executive_cost?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_spotlabel?.IncrementToken(indexEqualToOrGreaterThan, count);
        font_text_size?.IncrementToken(indexEqualToOrGreaterThan, count);
        fullbody_speak?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_easy_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_hard_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_luna_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        picture_center?.IncrementToken(indexEqualToOrGreaterThan, count);
        raise_yabo_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_item?.IncrementToken(indexEqualToOrGreaterThan, count);
        t_sound_volume?.IncrementToken(indexEqualToOrGreaterThan, count);
        target_way_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_menu_top?.IncrementToken(indexEqualToOrGreaterThan, count);
        trust_down_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_drain_mul?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_keep_form?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_level_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_status_hp?.IncrementToken(indexEqualToOrGreaterThan, count);
        b_sound_itv_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        b_sound_itv_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        bold_escapepath?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_blind_alpha?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_prepare_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_prepare_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_retreat_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_spotlabel?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_peace?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_league_up?.IncrementToken(indexEqualToOrGreaterThan, count);
        fullbody_detail?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_action_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_escape_run?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_hand_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_keep_color?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_poison_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_search_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_status_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_status_rec?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_suck_limit?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_summon_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_view_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        watching_replay?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_gain_lose?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_price_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_breast_width?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_gate_through?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_nocastle_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        btl_replace_line?.IncrementToken(indexEqualToOrGreaterThan, count);
        circle_alpha_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        color_spotlabel2?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_bestfriend?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_budget_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_most_power?.IncrementToken(indexEqualToOrGreaterThan, count);
        employ_price_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        loyal_escape_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        mode_normal_text?.IncrementToken(indexEqualToOrGreaterThan, count);
        raise_most_power?.IncrementToken(indexEqualToOrGreaterThan, count);
        scenario_select2?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_assist?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_leader?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_menu_right?.IncrementToken(indexEqualToOrGreaterThan, count);
        title_menu_space?.IncrementToken(indexEqualToOrGreaterThan, count);
        training_average?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_battle_cram?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_castle_cram?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_image_right?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_knock_speed?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_sword_slide?.IncrementToken(indexEqualToOrGreaterThan, count);
        vassal_price_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_money_mini?.IncrementToken(indexEqualToOrGreaterThan, count);
        arbeit_vassal_coe?.IncrementToken(indexEqualToOrGreaterThan, count);
        b_sound_no_repeat?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_frd?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_leg?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_sup?.IncrementToken(indexEqualToOrGreaterThan, count);
        skillicon_special?.IncrementToken(indexEqualToOrGreaterThan, count);
        spot_escape_route?.IncrementToken(indexEqualToOrGreaterThan, count);
        support_gain_lose?.IncrementToken(indexEqualToOrGreaterThan, count);
        support_trust_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_attack_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_charge_slide?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_element_heal?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_escape_range?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_status_death?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_summon_level?.IncrementToken(indexEqualToOrGreaterThan, count);
        compati_vassal_bdr?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_turndown?.IncrementToken(indexEqualToOrGreaterThan, count);
        diplo_sup_turndown?.IncrementToken(indexEqualToOrGreaterThan, count);
        neutral_member_max?.IncrementToken(indexEqualToOrGreaterThan, count);
        neutral_member_min?.IncrementToken(indexEqualToOrGreaterThan, count);
        trust_down_support?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_illusion_miss?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_missile_slide?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_retreat_speed?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_confusion_miss?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_lifebar_height?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_retreat_damage?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_specialuse_per?.IncrementToken(indexEqualToOrGreaterThan, count);
        unit_castle_forcefire?.IncrementToken(indexEqualToOrGreaterThan, count);
    }

    public void DecrementToken(uint indexEqualToOrGreaterThan, uint count)
    {
        if (Kind >= indexEqualToOrGreaterThan)
        {
            Kind -= count;
        }

        if (BracketLeft >= indexEqualToOrGreaterThan)
        {
            BracketLeft -= count;
        }

        if (BracketRight >= indexEqualToOrGreaterThan)
        {
            BracketRight -= count;
        }
        _war?.DecrementToken(indexEqualToOrGreaterThan, count);
        camp?.DecrementToken(indexEqualToOrGreaterThan, count);
        _help?.DecrementToken(indexEqualToOrGreaterThan, count);
        _load?.DecrementToken(indexEqualToOrGreaterThan, count);
        _menu?.DecrementToken(indexEqualToOrGreaterThan, count);
        _name?.DecrementToken(indexEqualToOrGreaterThan, count);
        _save?.DecrementToken(indexEqualToOrGreaterThan, count);
        _spot?.DecrementToken(indexEqualToOrGreaterThan, count);
        _tool?.DecrementToken(indexEqualToOrGreaterThan, count);
        _war2?.DecrementToken(indexEqualToOrGreaterThan, count);
        bloom?.DecrementToken(indexEqualToOrGreaterThan, count);
        _corps?.DecrementToken(indexEqualToOrGreaterThan, count);
        _diplo?.DecrementToken(indexEqualToOrGreaterThan, count);
        _help2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _load2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _menu2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _merce?.DecrementToken(indexEqualToOrGreaterThan, count);
        _name2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _save2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _spot2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _tool2?.DecrementToken(indexEqualToOrGreaterThan, count);
        discus?.DecrementToken(indexEqualToOrGreaterThan, count);
        font10?.DecrementToken(indexEqualToOrGreaterThan, count);
        font12?.DecrementToken(indexEqualToOrGreaterThan, count);
        font14?.DecrementToken(indexEqualToOrGreaterThan, count);
        font16?.DecrementToken(indexEqualToOrGreaterThan, count);
        font20?.DecrementToken(indexEqualToOrGreaterThan, count);
        member?.DecrementToken(indexEqualToOrGreaterThan, count);
        _battle?.DecrementToken(indexEqualToOrGreaterThan, count);
        _corps2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _detail?.DecrementToken(indexEqualToOrGreaterThan, count);
        _dialog?.DecrementToken(indexEqualToOrGreaterThan, count);
        _diplo2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _merce2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _spot22?.DecrementToken(indexEqualToOrGreaterThan, count);
        _status?.DecrementToken(indexEqualToOrGreaterThan, count);
        _battle2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _bstatus?.DecrementToken(indexEqualToOrGreaterThan, count);
        _commenu?.DecrementToken(indexEqualToOrGreaterThan, count);
        _detail2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _dialog2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _message?.DecrementToken(indexEqualToOrGreaterThan, count);
        _status2?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_auto?.DecrementToken(indexEqualToOrGreaterThan, count);
        gain_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        loyal_up?.DecrementToken(indexEqualToOrGreaterThan, count);
        max_unit?.DecrementToken(indexEqualToOrGreaterThan, count);
        my_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        npm_play?.DecrementToken(indexEqualToOrGreaterThan, count);
        politics?.DecrementToken(indexEqualToOrGreaterThan, count);
        raid_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        raid_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        raid_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        raid_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_red?.DecrementToken(indexEqualToOrGreaterThan, count);
        trust_up?.DecrementToken(indexEqualToOrGreaterThan, count);
        win_gain?.DecrementToken(indexEqualToOrGreaterThan, count);
        _bstatus2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _commenu2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _heromenu?.DecrementToken(indexEqualToOrGreaterThan, count);
        _message2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _scenario?.DecrementToken(indexEqualToOrGreaterThan, count);
        actor_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_limit?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_red?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_face?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_file?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_hp_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_mp_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_easy?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_hard?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_luna?.DecrementToken(indexEqualToOrGreaterThan, count);
        narabijun?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_blue?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_cyan?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_bgm?.DecrementToken(indexEqualToOrGreaterThan, count);
        wnd_alpha?.DecrementToken(indexEqualToOrGreaterThan, count);
        world_bgm?.DecrementToken(indexEqualToOrGreaterThan, count);
        _heromenu2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _powerinfo?.DecrementToken(indexEqualToOrGreaterThan, count);
        _scenario2?.DecrementToken(indexEqualToOrGreaterThan, count);
        battle_bgm?.DecrementToken(indexEqualToOrGreaterThan, count);
        bgm_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_healer?.DecrementToken(indexEqualToOrGreaterThan, count);
        cavalry_ai?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_blue?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_cyan?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_name?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        damage_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        damage_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_down?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_title?.DecrementToken(indexEqualToOrGreaterThan, count);
        force_fire?.DecrementToken(indexEqualToOrGreaterThan, count);
        free_raise?.DecrementToken(indexEqualToOrGreaterThan, count);
        loyal_down?.DecrementToken(indexEqualToOrGreaterThan, count);
        quota_view?.DecrementToken(indexEqualToOrGreaterThan, count);
        race_label?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_alpha?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_green?.DecrementToken(indexEqualToOrGreaterThan, count);
        roam_avoid?.DecrementToken(indexEqualToOrGreaterThan, count);
        senior_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        spot_fire1?.DecrementToken(indexEqualToOrGreaterThan, count);
        spot_fire2?.DecrementToken(indexEqualToOrGreaterThan, count);
        spot_route?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_name?.DecrementToken(indexEqualToOrGreaterThan, count);
        wnd_button?.DecrementToken(indexEqualToOrGreaterThan, count);
        wskin_cell?.DecrementToken(indexEqualToOrGreaterThan, count);
        zone_alpha?.DecrementToken(indexEqualToOrGreaterThan, count);
        _personinfo?.DecrementToken(indexEqualToOrGreaterThan, count);
        _powerinfo2?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_gain?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_turn?.DecrementToken(indexEqualToOrGreaterThan, count);
        battle_fast?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_counter?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_limit_c?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_minimap?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_msgtime?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_slowper?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_standby?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_unitmax?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_wingmax?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_green?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_white?.DecrementToken(indexEqualToOrGreaterThan, count);
        compati_bad?.DecrementToken(indexEqualToOrGreaterThan, count);
        damage_max2?.DecrementToken(indexEqualToOrGreaterThan, count);
        damage_min2?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_detail?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_move_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        item_window?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_normal?.DecrementToken(indexEqualToOrGreaterThan, count);
        neutral_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        neutral_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        power_order?.DecrementToken(indexEqualToOrGreaterThan, count);
        race_suffix?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_orange?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_purple?.DecrementToken(indexEqualToOrGreaterThan, count);
        ring_yellow?.DecrementToken(indexEqualToOrGreaterThan, count);
        senior_cost?.DecrementToken(indexEqualToOrGreaterThan, count);
        sound_count?.DecrementToken(indexEqualToOrGreaterThan, count);
        talent_mode?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_bgm_s?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_vanish?.DecrementToken(indexEqualToOrGreaterThan, count);
        zone_radius?.DecrementToken(indexEqualToOrGreaterThan, count);
        _personinfo2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _powerselect?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_mode_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_raidback?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_semiauto?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_orange?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_yellow?.DecrementToken(indexEqualToOrGreaterThan, count);
        dead_penalty?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_up?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_sup_up?.DecrementToken(indexEqualToOrGreaterThan, count);
        employ_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_balloon?.DecrementToken(indexEqualToOrGreaterThan, count);
        foreign_test?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_hprec_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_level_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_speed_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        leave_period?.DecrementToken(indexEqualToOrGreaterThan, count);
        loyal_border?.DecrementToken(indexEqualToOrGreaterThan, count);
        merits_bonus?.DecrementToken(indexEqualToOrGreaterThan, count);
        movement_num?.DecrementToken(indexEqualToOrGreaterThan, count);
        myhelp_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        picture_fade?.DecrementToken(indexEqualToOrGreaterThan, count);
        quarter_view?.DecrementToken(indexEqualToOrGreaterThan, count);
        skill_offset?.DecrementToken(indexEqualToOrGreaterThan, count);
        sound_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        support_gain?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        trust_morale?.DecrementToken(indexEqualToOrGreaterThan, count);
        unicode_test?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_use_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        war_capacity?.DecrementToken(indexEqualToOrGreaterThan, count);
        world_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        _personselect?.DecrementToken(indexEqualToOrGreaterThan, count);
        _powerselect2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _scenariotext?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_player?.DecrementToken(indexEqualToOrGreaterThan, count);
        b_sound_count?.DecrementToken(indexEqualToOrGreaterThan, count);
        battle_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        bold_spotpath?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_intercept?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_lineshift?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_minimap_w?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_detail1?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_detail2?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_detail3?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_magenta?.DecrementToken(indexEqualToOrGreaterThan, count);
        damage_consti?.DecrementToken(indexEqualToOrGreaterThan, count);
        event_bg_size?.DecrementToken(indexEqualToOrGreaterThan, count);
        executive_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        face_colorkey?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_prologue?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_attack_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_consti_mul?.DecrementToken(indexEqualToOrGreaterThan, count);
        fv_summon_mul?.DecrementToken(indexEqualToOrGreaterThan, count);
        item_sell_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        newpower_flag?.DecrementToken(indexEqualToOrGreaterThan, count);
        notalent_down?.DecrementToken(indexEqualToOrGreaterThan, count);
        picture_floor?.DecrementToken(indexEqualToOrGreaterThan, count);
        picture_trans?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_add?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_all?.DecrementToken(indexEqualToOrGreaterThan, count);
        spot_capacity?.DecrementToken(indexEqualToOrGreaterThan, count);
        support_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_scream_m?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_scream_n?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_slow_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        _personselect2?.DecrementToken(indexEqualToOrGreaterThan, count);
        _scenariotext2?.DecrementToken(indexEqualToOrGreaterThan, count);
        b_sound_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        bold_spotlabel?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_attackmode?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_battlemode?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_castle_lot?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_chargemode?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_min_damage?.DecrementToken(indexEqualToOrGreaterThan, count);
        caution_adjust?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_namehelp?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_prologue?.DecrementToken(indexEqualToOrGreaterThan, count);
        default_ending?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_cost_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_down?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_mode_itv?.DecrementToken(indexEqualToOrGreaterThan, count);
        executive_cost?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_spotlabel?.DecrementToken(indexEqualToOrGreaterThan, count);
        font_text_size?.DecrementToken(indexEqualToOrGreaterThan, count);
        fullbody_speak?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_easy_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_hard_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_luna_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        picture_center?.DecrementToken(indexEqualToOrGreaterThan, count);
        raise_yabo_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_item?.DecrementToken(indexEqualToOrGreaterThan, count);
        t_sound_volume?.DecrementToken(indexEqualToOrGreaterThan, count);
        target_way_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_menu_top?.DecrementToken(indexEqualToOrGreaterThan, count);
        trust_down_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_drain_mul?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_keep_form?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_level_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_status_hp?.DecrementToken(indexEqualToOrGreaterThan, count);
        b_sound_itv_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        b_sound_itv_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        bold_escapepath?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_blind_alpha?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_prepare_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_prepare_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_retreat_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_spotlabel?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_peace?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_league_up?.DecrementToken(indexEqualToOrGreaterThan, count);
        fullbody_detail?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_action_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_escape_run?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_hand_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_keep_color?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_poison_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_search_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_status_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_status_rec?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_suck_limit?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_summon_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_view_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        watching_replay?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_gain_lose?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_price_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_breast_width?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_gate_through?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_nocastle_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        btl_replace_line?.DecrementToken(indexEqualToOrGreaterThan, count);
        circle_alpha_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        color_spotlabel2?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_bestfriend?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_budget_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_most_power?.DecrementToken(indexEqualToOrGreaterThan, count);
        employ_price_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        loyal_escape_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        mode_normal_text?.DecrementToken(indexEqualToOrGreaterThan, count);
        raise_most_power?.DecrementToken(indexEqualToOrGreaterThan, count);
        scenario_select2?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_assist?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_leader?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_menu_right?.DecrementToken(indexEqualToOrGreaterThan, count);
        title_menu_space?.DecrementToken(indexEqualToOrGreaterThan, count);
        training_average?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_battle_cram?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_castle_cram?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_image_right?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_knock_speed?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_sword_slide?.DecrementToken(indexEqualToOrGreaterThan, count);
        vassal_price_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_money_mini?.DecrementToken(indexEqualToOrGreaterThan, count);
        arbeit_vassal_coe?.DecrementToken(indexEqualToOrGreaterThan, count);
        b_sound_no_repeat?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_frd?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_leg?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_success_sup?.DecrementToken(indexEqualToOrGreaterThan, count);
        skillicon_special?.DecrementToken(indexEqualToOrGreaterThan, count);
        spot_escape_route?.DecrementToken(indexEqualToOrGreaterThan, count);
        support_gain_lose?.DecrementToken(indexEqualToOrGreaterThan, count);
        support_trust_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_attack_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_charge_slide?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_element_heal?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_escape_range?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_status_death?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_summon_level?.DecrementToken(indexEqualToOrGreaterThan, count);
        compati_vassal_bdr?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_frd_turndown?.DecrementToken(indexEqualToOrGreaterThan, count);
        diplo_sup_turndown?.DecrementToken(indexEqualToOrGreaterThan, count);
        neutral_member_max?.DecrementToken(indexEqualToOrGreaterThan, count);
        neutral_member_min?.DecrementToken(indexEqualToOrGreaterThan, count);
        trust_down_support?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_illusion_miss?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_missile_slide?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_retreat_speed?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_confusion_miss?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_lifebar_height?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_retreat_damage?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_specialuse_per?.DecrementToken(indexEqualToOrGreaterThan, count);
        unit_castle_forcefire?.DecrementToken(indexEqualToOrGreaterThan, count);
    }
}
